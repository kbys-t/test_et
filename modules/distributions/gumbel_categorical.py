...
class GumbelCategorical(Distribution):
    r"""
    ...
    Example::
        >>> m = GumbelCategorical(torch.tensor([ 0.25, 0.25, 0.25, 0.25 ]))
    ...
    def __init__(self, probs=None, logits=None, validate_args=None):
        ...
        super(GumbelCategorical, self).__init__(batch_shape, validate_args=validate_args)

    def expand(self, batch_shape, _instance=None):
        new = self._get_checked_instance(GumbelCategorical, _instance)
        ...
        super(GumbelCategorical, new).__init__(batch_shape, validate_args=False)
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        probs = self.probs.expand(param_shape)
        probs_2d = probs.reshape(-1, self._num_events)
        eps = torch.zeros(probs_2d.size(), dtype=probs_2d.dtype, device=probs_2d.device).uniform_(1e-32, 1)
        return torch.argmax(-torch.log(-torch.log(eps)) + torch.log(probs_2d), dim=1).reshape(sample_shape)
